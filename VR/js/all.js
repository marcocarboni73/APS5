...
function s_cramble(options)
{
	options = options || {};
	var images = new Array();
	var directory_path = options['directory'];
	var images_prefix  = options['prefix'] || "";
	var images_extension = options['extension'] || "jpg";
	var images_count = options['frames'] || 20;
	var img_id = options['id'] || "3dwave-image";
	var animate = options['animation'] || false;
	var rows = options['rows'] || 1;
	var speed_manual = options['speed'] || 0.3;
	var padding = options['padding'] || 0;
	var loops = options['loops'] || false;
	var rebound = options['rebound'] || 0.5;
	

	for(var i=1; i <= images_count*rows; ++i)
	{
		var padded = pad(i,padding);
		images[i-1] = directory_path + "/" + images_prefix +padded+"."+images_extension;
	}
	var speed = (animate ? speed_manual : 0.0);
	speed = speed*(-1.0);
	
	if(rows == 1)
	{
		$("#"+img_id).reel({frames:images_count,images: images, cw:true, laziness:3.0, speed: speed,preloader:16, loops: loops, rebound: rebound}); 		
	}
	
	else
	{
		$("#"+img_id).reel({frames:images_count,images: images, cw:true, laziness:3.0, speed: speed,preloader:16,
							rows: rows,loops: loops, rebound : rebound}); 		
	}
}
            ...
            while(preload.length){
              var
                ...
                $img= $(new Image()).bind('load'+ns, function update_preloader(){
                  img_tag.preloaded++
                  $(this).unbind(ns);
				  var xxxx = img_tag.preloaded*100 / img_tag.frames;
				  $preloader.html("<div>" + Math.floor(xxxx) + "%</div>");
